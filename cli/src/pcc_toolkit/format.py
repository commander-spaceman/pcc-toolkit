...
def validation_summary(report: dict) -> None:
    ...
    console.print()
    console.print(f"  Total: {s.get('total', 0)}  "
    ...
    for r in report.get("results", []):
        ...
        console.print(f"  [{sc}]{icon} {r['conversation_id']} (#{r['export_index']}) — {status}[/{sc}]")
...
def evidence_report(report: dict) -> None:
    ...
    for ev in report.get("evidence", []):
        ...
        if t1:
            ...
            for hit in t1:
                console.print(f"    {hit.get('conversation_id', '?')} in {hit.get('file_path', '?')}")

        t2 = ev.get("semantic_container", [])
        if t2:
            console.print(f"  [yellow]Tier 2 — Semantic container[/yellow] ({len(t2)}):")
            for hit in t2:
                console.print(f"    {hit.get('export_name', '?')} [{hit.get('class_name', '?')}]")
...
def batch_summary(report: dict) -> None:
    ...
    console.print(f"Files found: {report.get('files_found', 0)}  "
                  f"OK: [green]{report.get('files_ok', 0)}[/green]  "
                  f"Errors: [red]{report.get('files_error', 0)}[/red]")
    if report.get("total_conversations"):
        console.print(f"Conversations: {report.get('total_conversations', 0)}  "
